...
# ------------------------------------------------------------
# CASE-SPECIFIC SUMMER RHS FOR PHYTOPLANKTON MORTALITY
# ------------------------------------------------------------
def make_rhs_summer_mP(m_P_case):
    def rhs_summer_mP(t, y):
        N    = y[iN]
        P    = y[iP]
        Z    = y[iZ]
        D    = y[iD]
    ...
    return rhs_summer_mP


# ------------------------------------------------------------
# RUN TWO PHYTOPLANKTON MORTALITY CASES
# ------------------------------------------------------------
# keep nutrient loading fixed
N_surface_fixed = 5.0
y0_case = make_initial_state(N_surface_fixed)

# choose two phytoplankton mortality rates
m_P_low = 0.25
m_P_high = 0.35

rhs_low_mP = make_rhs_summer_mP(m_P_low)
rhs_high_mP = make_rhs_summer_mP(m_P_high)

sol_low_mP = solve_ivp(
    rhs_low_mP, t_span_ss, y0_case, t_eval=t_eval_ss,
    method="BDF", rtol=1e-7, atol=1e-10, max_step=1.0
)

sol_high_mP = solve_ivp(
    rhs_high_mP, t_span_ss, y0_case, t_eval=t_eval_ss,
    method="BDF", rtol=1e-7, atol=1e-10, max_step=1.0
)
...
# ------------------------------------------------------------
N_lowm  = sol_low_mP.y[iN, -1]
P_lowm  = sol_low_mP.y[iP, -1]
D_lowm  = sol_low_mP.y[iD, -1]
O_lowm  = sol_low_mP.y[iO, -1]

N_highm = sol_high_mP.y[iN, -1]
P_highm = sol_high_mP.y[iP, -1]
D_highm = sol_high_mP.y[iD, -1]
O_highm = sol_high_mP.y[iO, -1]
...
# Nutrients
axs[0].plot(N_lowm, z, color="darkblue", lw=2)
axs[0].plot(N_highm, z, color="darkblue", lw=2, ls="--")
...
# Oxygen
axs[1].plot(O_lowm, z, color="black", lw=2)
axs[1].plot(O_highm, z, color="black", lw=2, ls="--")
axs[1].set_title("Oxygen")
axs[1].set_xlabel("O$_2$ [mmol O$_2$ m$^{-3}$]")
axs[1].grid(True)

# Phytoplankton
axs[2].plot(P_lowm, z, color="green", lw=2)
axs[2].plot(P_highm, z, color="green", lw=2, ls="--")
axs[2].set_title("Phytoplankton")
axs[2].set_xlabel("P [mmol N m$^{-3}$]")
axs[2].grid(True)

# Detritus
axs[3].plot(D_lowm, z, color="red", lw=2)
axs[3].plot(D_highm, z, color="red", lw=2, ls="--")
...
fig.legend(
    legend_lines,
    [fr"Low $m_P$ = {m_P_low} d$^{{-1}}$", fr"High $m_P$ = {m_P_high} d$^{{-1}}$"],
