...
def bottom_flux(tracer_name, C_bottom, sed=None, w_D_bottom=None):
    ...
    if tracer_name == "N":
        ...
        if w_D_bottom is None:
            w_D_bottom = W[3]
        return w_D_bottom * C_bottom

    return 0.0

def vertical_transport(C, kappa_interface, w=0.0, tracer_name="", sed=None, w_D_bottom=None):
    """
    ...
    J[nz] = bottom_flux(tracer_name, C[-1], sed=sed, w_D_bottom=w_D_bottom)
...
# ------------------------------------------------------------
# CASE-SPECIFIC SUMMER RHS FOR DETRITUS SINKING SPEED
# ------------------------------------------------------------
def make_rhs_summer_wD(w_D_case):
    def rhs_summer_wD(t, y):
        N    = y[iN]
        P    = y[iP]
        Z    = y[iZ]
        D    = y[iD]
        ...
        # boundary fluxes with case-specific detritus sinking
        J_D_bot = bottom_flux("D", D[-1], w_D_bottom=w_D_case)
        J_N_bot = bottom_flux("N", N[-1], sed=Nsed)
        ...
        dP = vertical_transport(P, kappa_interface, w=W[1],     tracer_name="P") + dP_reac
        dZ = vertical_transport(Z, kappa_interface, w=W[2],     tracer_name="Z") + dZ_reac
        dD = vertical_transport(D, kappa_interface, w=w_D_case, tracer_name="D",w_D_bottom=w_D_case) + dD_reac
        dO = vertical_transport(O, kappa_interface, w=W[4],     tracer_name="O") + dO_reac
    ...
    return rhs_summer_wD


# ------------------------------------------------------------
# RUN TWO DETRITUS SINKING SPEED CASES
# ------------------------------------------------------------
# keep nutrient loading fixed
N_surface_fixed = 5.0
y0_case = make_initial_state(N_surface_fixed)

# choose two detritus sinking speeds
w_D_low = 4.0
w_D_high = 6.0

rhs_low_wD = make_rhs_summer_wD(w_D_low)
rhs_high_wD = make_rhs_summer_wD(w_D_high)

sol_low_wD = solve_ivp(
    rhs_low_wD, t_span_ss, y0_case, t_eval=t_eval_ss,
    method="BDF", rtol=1e-7, atol=1e-10, max_step=1.0
)

sol_high_wD = solve_ivp(
    rhs_high_wD, t_span_ss, y0_case, t_eval=t_eval_ss,
    method="BDF", rtol=1e-7, atol=1e-10, max_step=1.0
)
...
# ------------------------------------------------------------
N_loww  = sol_low_wD.y[iN, -1]
P_loww  = sol_low_wD.y[iP, -1]
D_loww  = sol_low_wD.y[iD, -1]
O_loww  = sol_low_wD.y[iO, -1]

N_highw = sol_high_wD.y[iN, -1]
P_highw = sol_high_wD.y[iP, -1]
D_highw = sol_high_wD.y[iD, -1]
O_highw = sol_high_wD.y[iO, -1]


# ------------------------------------------------------------
# FOUR PANEL STEADY-STATE COMPARISON
# ------------------------------------------------------------
fig, axs = plt.subplots(1, 4, figsize=(14,6), sharey=True)

# Nutrients
axs[0].plot(N_loww, z, color="darkblue", lw=2)
axs[0].plot(N_highw, z, color="darkblue", lw=2, ls="--")
...
# Oxygen
axs[1].plot(O_loww, z, color="black", lw=2)
axs[1].plot(O_highw, z, color="black", lw=2, ls="--")
axs[1].set_title("Oxygen")
axs[1].set_xlabel("O$_2$ [mmol O$_2$ m$^{-3}$]")
axs[1].grid(True)

# Phytoplankton
axs[2].plot(P_loww, z, color="green", lw=2)
axs[2].plot(P_highw, z, color="green", lw=2, ls="--")
axs[2].set_title("Phytoplankton")
axs[2].set_xlabel("P [mmol N m$^{-3}$]")
axs[2].grid(True)

# Detritus
axs[3].plot(D_loww, z, color="red", lw=2)
axs[3].plot(D_highw, z, color="red", lw=2, ls="--")
...
fig.legend(
    legend_lines,
    [fr"Low $w_D$ = {w_D_low} m d$^{{-1}}$", fr"High $w_D$ = {w_D_high} m d$^{{-1}}$"],
