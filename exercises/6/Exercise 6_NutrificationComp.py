...
# ------------------------------------------------------------
# 1) STEADY SUMMER SETUP
# ------------------------------------------------------------
# We force constant summer conditions:
# - no seasonality
# - strong light
# - relatively weak stratified mixing
# - no self-shading in the forcing profile
#
...
def getLIGHTandKAPPAS_summer(t, P=None, D=None):
    ...
    Fixed summer conditions:
      - low mixing near surface/bottom
      - high light
    """
    # --- fixed summer diffusivity profile ---
    kappa_top = 6.0
    kappa_bottom = 3.0
    z_mix = 0.1 * depth
    zeta_mix = 5.0
...
# ------------------------------------------------------------
# 2) HELPER TO BUILD INITIAL CONDITIONS FOR EACH N LOADING
# ------------------------------------------------------------
def make_initial_state(N_surface):
    """
    Build one initial condition set.
    N_surface controls nutrient loading level.
    """
...
# ------------------------------------------------------------
# 3) RUN LOW- AND HIGH-NUTRIENT CASES
# ------------------------------------------------------------
# Choose the two nutrient loading cases here
N_surface_low = 10.0
N_surface_high = 80.0

y0_low = make_initial_state(N_surface_low)
y0_high = make_initial_state(N_surface_high)

sol_low = solve_ivp(
    rhs_summer, t_span_ss, y0_low, t_eval=t_eval_ss,
    method="BDF", rtol=1e-7, atol=1e-10, max_step=1.0
)

sol_high = solve_ivp(
    rhs_summer, t_span_ss, y0_high, t_eval=t_eval_ss,
    method="BDF", rtol=1e-7, atol=1e-10, max_step=1.0
)
...
N_low  = sol_low.y[iN, -1]
P_low  = sol_low.y[iP, -1]
D_low  = sol_low.y[iD, -1]
O_low  = sol_low.y[iO, -1]

N_high = sol_high.y[iN, -1]
P_high = sol_high.y[iP, -1]
D_high = sol_high.y[iD, -1]
O_high = sol_high.y[iO, -1]


# ============================================================
# FOUR PANEL STEADY STATE COMPARISON (1x4 layout)
# ============================================================
...
# Nutrients
# ------------------------------------------------------------
axs[0].plot(N_low, z, color="darkblue", lw=2)
axs[0].plot(N_high, z, color="darkblue", lw=2, ls="--")
...
# Oxygen
# ------------------------------------------------------------
axs[1].plot(O_low, z, color="black", lw=2)
axs[1].plot(O_high, z, color="black", lw=2, ls="--")
...
# Phytoplankton
# ------------------------------------------------------------
axs[2].plot(P_low, z, color="green", lw=2)
axs[2].plot(P_high, z, color="green", lw=2, ls="--")
...
# Detritus
# ------------------------------------------------------------
axs[3].plot(D_low, z, color="red", lw=2)
axs[3].plot(D_high, z, color="red", lw=2, ls="--")
...
fig.legend(
    legend_lines,
    ["Low nutrient loading", "High nutrient loading"],
    bbox_to_anchor=(0.5, 0.97),   # moves legend down
    loc="upper center",
    ncol=2,
    frameon=True
)
