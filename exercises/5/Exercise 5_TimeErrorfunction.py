...
P0 = 7.0 * np.exp(-z / 3.0) + 0.01   # initial phytoplankton        [mmol N m^-3]
Z0 = 10 * np.exp(-z / 5.0) + 0.02   # initial zooplankton          [mmol N m^-3]
D0 = + 1.5 * (z / depth)         # initial detritus             [mmol N m^-3]
...
# 3) TIME SETUP
# ============================================================
years = 10                                  # simulation length       [years]
...
# ============================================================
# 17) YEARLY MEAN SPIN-UP DIAGNOSTIC (vs final year)
# ============================================================

def relative_error_scalar(val, ref, eps=1e-14):
    return np.abs(val - ref) / (np.abs(ref) + eps)

def relative_l2_profile(a, b, eps=1e-14):
    num = np.sqrt(np.mean((a - b)**2))
    den = np.sqrt(np.mean(b**2)) + eps
    return num / den

# ------------------------------------------------------------
# 17A) Compute yearly mean diagnostics
# ------------------------------------------------------------
n_years = int(np.floor(sol.t[-1] / 365.0))
year_axis = np.arange(1, n_years + 1)

N_year_mean = np.zeros(n_years)
P_year_mean = np.zeros(n_years)
Z_year_mean = np.zeros(n_years)
D_year_mean = np.zeros(n_years)
O_bottom_year_mean = np.zeros(n_years)

P_profile_year_mean = np.zeros((n_years, nz))
O_profile_year_mean = np.zeros((n_years, nz))
...
for yr in range(n_years):
    ...
    # depth-integrated yearly means
    N_int = np.sum(sol.y[iN, :][:, mask], axis=0) * dz
    P_int = np.sum(sol.y[iP, :][:, mask], axis=0) * dz
    Z_int = np.sum(sol.y[iZ, :][:, mask], axis=0) * dz
    D_int = np.sum(sol.y[iD, :][:, mask], axis=0) * dz

    N_year_mean[yr] = np.mean(N_int)
    P_year_mean[yr] = np.mean(P_int)
    Z_year_mean[yr] = np.mean(Z_int)
    D_year_mean[yr] = np.mean(D_int)

    # bottom oxygen yearly mean
    O_bottom_year_mean[yr] = np.mean(sol.y[iO, :][-1, mask])

    # full yearly mean profiles
    P_profile_year_mean[yr, :] = np.mean(sol.y[iP, :][:, mask], axis=1)
    O_profile_year_mean[yr, :] = np.mean(sol.y[iO, :][:, mask], axis=1)

# ------------------------------------------------------------
# 17B) Compare each year to the final reference year
# ------------------------------------------------------------
N_ref = N_year_mean[-1]
P_ref = P_year_mean[-1]
Z_ref = Z_year_mean[-1]
D_ref = D_year_mean[-1]
O_bottom_ref = O_bottom_year_mean[-1]

P_prof_ref = P_profile_year_mean[-1, :]
O_prof_ref = O_profile_year_mean[-1, :]

N_err = np.array([relative_error_scalar(v, N_ref) for v in N_year_mean])
P_err = np.array([relative_error_scalar(v, P_ref) for v in P_year_mean])
Z_err = np.array([relative_error_scalar(v, Z_ref) for v in Z_year_mean])
D_err = np.array([relative_error_scalar(v, D_ref) for v in D_year_mean])
O_bottom_err = np.array([relative_error_scalar(v, O_bottom_ref) for v in O_bottom_year_mean])

P_prof_err = np.array([relative_l2_profile(P_profile_year_mean[k, :], P_prof_ref) for k in range(n_years)])
O_prof_err = np.array([relative_l2_profile(O_profile_year_mean[k, :], O_prof_ref) for k in range(n_years)])
...
plt.plot(year_axis, P_err, marker="o", lw=2, color="green", label="Integrated P")
plt.plot(year_axis, O_bottom_err, marker="o", lw=2, color="black", label="Bottom O$_2$")
plt.plot(year_axis, P_prof_err, marker="o", lw=2, ls="--", color="limegreen", label="P profile")
plt.plot(year_axis, O_prof_err, marker="o", lw=2, ls="--", color="gray", label="O$_2$ profile")

plt.yscale("log")
plt.xlabel("Simulation year")
plt.ylabel("Relative error to final year [-]")

plt.grid(True, which="major", linestyle="-", linewidth=0.8)
plt.grid(True, which="minor", linestyle=":", linewidth=0.5)
plt.xlim([min(year_axis),max(year_axis-1)])
plt.ylim([1e-3,1e-1])
plt.legend()
plt.tight_layout()
plt.show()
